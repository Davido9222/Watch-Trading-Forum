...
export const FlappyWatchPage: React.FC = () => {
  ...
  // Leaderboards (fetched async from MongoDB via the API)
  const [highScoreLeaderboard, setHighScoreLeaderboard] = useState<Array<{ userId: string; username: string; avatar?: string; score: number }>>([]);
  const [totalScoreLeaderboard, setTotalScoreLeaderboard] = useState<Array<{ userId: string; username: string; avatar?: string; totalScore: number }>>([]);

  // Personal high score: prefer the value from the logged-in user record (always fresh),
  // fall back to the leaderboard entry. This makes "Your Best" reliable even before the
  // global leaderboard finishes loading.
  const personalHighScore = currentUser
    ? ((currentUser as any).flappyHighScore
        || highScoreLeaderboard.find(entry => entry.userId === currentUser.id)?.score
        || 0)
    : 0;
  ...
  const refreshLeaderboards = useCallback(async () => {
    try {
      const [hs, ts] = await Promise.all([
        getFlappyLeaderboard(),
        getFlappyTotalLeaderboard(),
      ]);
      if (Array.isArray(hs)) setHighScoreLeaderboard(hs);
      if (Array.isArray(ts)) setTotalScoreLeaderboard(ts);
    } catch {
      /* ignore — leaderboards stay as-is */
    }
  }, [getFlappyLeaderboard, getFlappyTotalLeaderboard]);

  // Fetch once on mount. We refresh again after every game over instead of polling
  // every 2s — the old polling was the main source of in-game lag.
  useEffect(() => {
    refreshLeaderboards();
  }, [refreshLeaderboards]);
  ...
  const handleGameOver = useCallback(() => {
    ...
    if (isAuthenticated && currentUser) {
      // Save the score to MongoDB then refresh the leaderboards.
      Promise.resolve(updateFlappyStats(currentUser.id, scoreRef.current))
        .then(() => refreshLeaderboards())
        .catch(() => { /* ignore */ });
    }
  }, [isAuthenticated, currentUser, updateFlappyStats, refreshLeaderboards]);

...
const SettingsTab: React.FC = () => {
  ...
  // 2FA handlers — must await; the store calls hit the backend asynchronously.
  const handleEnable2FA = async () => {
    setTwoFAStatus(null);
    try {
      const result: any = await enable2FA();
      ...
      setTwoFAStatus({ type: 'error', msg: err?.message || 'Failed to enable 2FA' });
    }
  };
  ...
  const handleVerify2FA = async () => {
    ...
    try {
      const result: any = await verify2FA(twoFactorCode);
      if (result?.success) {
      ...
      setTwoFAStatus({ type: 'error', msg: err?.message || 'Invalid code' });
    }
  };
  ...
  const handleDisable2FA = async () => {
    setTwoFAStatus(null);
    try {
      const result: any = await disable2FA();
      if (result?.success) {
      ...
      setTwoFAStatus({ type: 'error', msg: err?.message || 'Failed to disable 2FA' });
    }
  };

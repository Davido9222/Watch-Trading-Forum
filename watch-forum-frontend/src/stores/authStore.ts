...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  awardBadge: (async (userId: string, badgeType: BadgeType) => {
    try {
      await api.post(`/users/${userId}/badges`, { type: badgeType });
      await get().refreshUsers();
      return { success: true };
    } catch (e: any) { return { success: false, error: e.message }; }
  }) as any,
  removeBadge: ((userId: string, badgeId: string) => {
    api.del(`/users/${userId}/badges/${badgeId}`)
      .then(() => get().refreshUsers())
      .catch(() => {});
  }) as any,
  ...
  assignDonorGif: ((userId: string, gifUrl: string) => {
    api.post(`/users/${userId}/donor-gif`, { url: gifUrl })
      .then(() => get().refreshUsers())
      .catch(() => {});
  }) as any,
  removeDonorGif: ((userId: string) => {
    api.del(`/users/${userId}/donor-gif`)
      .then(() => get().refreshUsers())
      .catch(() => {});
  }) as any,
  ...
  removeHallOfShame: (async (userId: string) => {
    try {
      await api.del(`/users/${userId}/hall-of-shame`);
      await get().refreshUsers();
      return { success: true };
    } catch (e: any) { return { success: false, error: e.message }; }
  }) as any,
